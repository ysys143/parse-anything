...
`ontology/` 레지스트리 디렉터리에 문서종류별 프로파일을 두고 버전관리한다. 사람이 읽고 문서화하기 좋으며,
...
> **실무 정본: [`docs/lightrag-integration-guide.md`](../lightrag-integration-guide.md)** — 이 절의 설계를
> E2E로 실측·수술해 검증된 두 경로(delimiter `ainsert` / `ainsert_custom_kg`+concept 융합) + 함정 표 +
> 재현 데모(`demo/docker/`)로 정본화한 실무 가이드. 아래는 그 설계·근거.
...
**실측 확인 (2026-07, 설치된 `lightrag-hku` 1.5.4 패키지의 실제 `chunking_by_token_size`를 산문+표 입력으로
4케이스 실행 + assertion 통과).** `split_by_character`가 경계를 전달한다 — 델리미터 없이 넣으면 seam을 무시하고
토큰 윈도우로 재단해 **우리 경계가 유실**되고(baseline에서 첫 단락이 통짜 보존 안 됨), 델리미터를 주면 청커가
**세그먼트 경계를 절대 넘지 않는다**(각 단락이 통짜 1청크로 보존, 큰 표만 내부 분할). 토크나이저는 무손실
문자단위로 대체(tiktoken BPE vocab이 네트워크 차단으로 오프라인 로드 불가) — 분기 로직은 토크나이저 무관이라
결론 동일, 토큰 *수치*만 다름. **재현 테스트: `tests/test_lightrag_chunk_boundary.py`**(설치된 lightrag로 4케이스
실행, 미설치 시 skip — 주장이 아니라 재실행 가능한 아티팩트).

- **`only=False` (1c)**: 각 seam 세그먼트가 경계로 보존되고 **oversize 세그먼트만** 내부적으로 토큰 재분할
  (overlap 적용). 에러 없음, seam 횡단 없음 → **안전한 경계 전달 경로.** 단 큰 원자(표)는 내부 절단됨 →
  **원자성 미보장.**
- **`only=True` (1b)**: 세그먼트와 1:1 정확 경계 + 원자성 보장. 단 **어떤 세그먼트가 `chunk_token_size`를
  초과하면 조용히 큰 청크를 내지 않고 `ChunkTokenLimitExceededError`를 던져 인제스트를 중단**시킨다(예전 문서의
  "큰 청크가 그대로 남아 윈도우 초과"는 오류 — 정정). → **1b에선 "oversize 원자만 미리 hard-split"이 선택이
  아니라 필수**(안 하면 인제스트 실패).

**(a)원자 불파괴 + (b)소비자 사이징 소유는 우리 청크가 이미 소비자 버짓에 맞을 때만 동시 성립.** 현실 절충:
prose는 미리 합리적 크기로 나눠 seam 힌트만 주고 **oversize 원자 단위는 hard-split**(1c에선 품질 위해, 1b에선
에러 회피 위해 — 하이브리드).
