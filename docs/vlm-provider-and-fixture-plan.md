...
초기 serving 구조는 provider client를 orchestration layer 뒤에 숨긴다. 전체 필수 계약은 `docs/pdf-pipeline-requirements.md`와 `docs/processing-tiers-and-adaptation.md`를 기준으로 하며, 이 문서는 provider와 fixture 관점에서 그 계약을 어떻게 검증할지 기록한다.

1. 입력 PDF를 고DPI 페이지 이미지와 1차 markdown/json 후보로 분해한다. 렌더링 기본 후보는 pypdfium2 계열이다.
2. ODL 또는 동등 결정론 추출로 텍스트, bbox, table region, semantic table 후보를 확보한다.
3. scan vs born-digital은 pdf-inspector classify와 텍스트 char 수를 함께 사용해 판정한다.
4. Processing-depth policy가 페이지별로 deterministic-only, PaddleOCR official API, Gemini direct, human-review 중 하나를 고른다. 기본은 deterministic-only이며, 스캔/그림·차트/인코딩 깨짐/산술 불변식 실패/결정론 불완전/방향 불확실 같은 명시 트리거가 있어야 escalation한다.
5. PaddleOCR official API와 Gemini direct는 같은 내부 IR로 정규화한다.
...
민감 문서는 provider I/O 제약도 route 결정에 넣는다. PaddleOCR official API 경로가 이미지 URL fetch만 지원하고 base64 inline image를 지원하지 않는 경우, 공개 호스팅이 필요한 민감 문서에는 부적합하다. 이 경우 Gemini inline image, private-network hosting, local/self-hosted OCR/VLM 같은 대체 경로가 필요하다.

## 5. Fixture and Golden-Set Strategy

초기 fixture는 두 층으로 둔다.

- Public generated fixtures: repo에 넣을 수 있는 합성 fixture다. 레이아웃, 언어, 표, 페이지 걸친 표, 이미지, 회전, 스캔 느낌, text-as-image, 저해상도, 숫자 guard 같은 실패 모드를 의도적으로 나눈다.
...
현재 repo manifest는 초기 8개 family로 시작하지만, 전체 필수 계약 검증에는 아래 목표 family coverage가 필요하다.
...
| `page_spanning_table` | 페이지에 걸친 표, continuation header | bbox 열좌표 기반 연속 판정과 logical table 병합 |
...
| `numeric_guard_table` | 산술 불변식이 있는 숫자 표 | source gate, oracle injection, hallucination 방어 |

현재 repo의 fixture manifest는 초기 8개 family만 metadata-only로 갖고 있다. 전체 필수 계약을 검증하려면 위 두 family 또는 동등한 document-level golden case가 추가되어야 한다. 골든셋은 페이지 단위 정답뿐 아니라 document-level continuation, table merge, source text oracle, arithmetic invariant, guard flag expectation을 포함해야 한다. 기존 page-level schema는 현재 slice의 최소 계약이다. 참고: `docs/parsing-engine-spike-plan.md:30`.

## 6. Fixture Sufficiency Criteria

Fixture set은 다음 기준을 만족해야 충분하다고 본다.

- Coverage: 단순 born-digital, 다단 한영 혼재, 병합셀/중첩헤더 표, 페이지에 걸친 표, 이미지 겹침, text-as-image, 산술 불변식이 있는 숫자 표, 수기 또는 회전 글자, 스캔 PDF, 비-PDF 후보가 최소 한 번씩 등장한다.
...
- Cost visibility: provider 호출 횟수, VLM 생략 페이지 수, processing-depth route, 비용을 fixture별로 집계할 수 있어야 한다.
- Guard visibility: source gate, arithmetic invariant, hallucination flag, human-review flag를 fixture별로 집계할 수 있어야 한다.
- Adaptation visibility: 도메인 보정 전후의 오류율, 비용, 사람검토량 변화를 비교할 수 있어야 한다.
- Privacy visibility: URL fetch provider가 민감 문서에서 배제되는지 검증할 수 있어야 한다.
...
- Processing-depth policy 변경이 비용 또는 지연뿐 아니라 품질, 숫자 정확도, hallucination flag에도 영향을 준다.
- Domain adaptation policy 변경이 오류율, 사람검토량, 비용 중 하나 이상을 의미 있게 바꾼다.
- 실패가 fixture family와 연결된다. 예를 들어 병합셀 표, text-as-image, 회전 글자 같은 원인이 분리되어야 한다.
- 기존 scorecard 항목인 읽기순서, 표 충실도, 페이지 걸친 표, 숫자 정확도, hallucination rate, 그림/다이어그램 서술, OCR 오염 여부, 라우팅 정확도, 비용, bbox 보존, md/json 정확성에 결과가 매핑된다. 참고: `docs/parsing-engine-spike-plan.md:75`.
