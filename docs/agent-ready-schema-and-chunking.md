...
**실측 확인 (2026-07, 실제 `lightrag/chunker/token_size.py` 소스로 4케이스 실행).** `split_by_character`가
경계를 전달한다 — 델리미터 없이 넣으면 seam을 무시하고 토큰 윈도우로 재단해 **우리 경계가 유실**되고, 델리미터를
주면 청커가 **세그먼트 경계를 절대 넘지 않는다.**
