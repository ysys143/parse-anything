...
class PageSignals:
    ...
    counting over-fires and pdf-inspector misses merged cells)."""
    text_chars: int
    table_rows: int
    image_count: int
...
def decide_route(signals: PageSignals, policy: TriagePolicy = TriagePolicy()) -> Route:
    ...
    Bias is toward VLM: a born-digital page that is misrouted to ``deterministic`` still
    keeps correct values via the text layer (F10), whereas under-serving a table loses
    structure -- so ``deterministic`` fires only for plain text with no table and no image.
    """
