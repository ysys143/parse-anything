...
| 6 | PaddlePaddle/PaddleOCR-VL | 0.9B | 0.859 | 2.75 | parse-anything's native paddle backend; single-shot = no LaTeX, loops on figures. needs its layout pipeline |
...
- **Deployment was most of the work** — each model needed a different serving fix (vLLM version pins,
  context caps, custom images). See `models.json` notes and the git history.
